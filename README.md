...
|  TreeHeight | [Solved](https://github.com/mughees-asif/hackerrank-solved-challenges/blob/master/algorithms/BinaryTreeDiameter/src/Node.java) |  :orange_book: | get `leftHeight` and `rightHeight` -> get `leftDiameter` and `rightDiameter` -> return `max(leftHeight + rightHeight, max(leftDiameter + rightDiameter))` |